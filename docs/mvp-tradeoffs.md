...
| Staging Ground | Milestone 8 defines a frontend-only local review surface that summarizes Stage Payload readiness, payload completeness, Token Budget, evidence inclusion, warnings, and limitations. It does not submit anything. AI review and redaction are shown as not implemented. | This makes the privacy and cost boundary visible before any model integration and prevents the MVP from implying that local evidence has already been approved for external submission. | No submission behavior, Safety Gate, redaction check, user approval flow, provider or model selection, prompt construction, Stage Report, backend logic, or persistence. | Add Safety Gate, redaction checks, explicit user approval before submission, model/provider selection, structured Stage Report generation, and persistent review history. |
