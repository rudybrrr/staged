...
| Staging Ground | Implements a frontend-only, local, read-only pre-submission review surface based on the current Stage Payload and Token Budget. It summarizes readiness, payload completeness, selected-file diff evidence, command result evidence, warnings, and limitations. It shows AI review, secret redaction, and Safety Gate enforcement as blocked/not implemented. It does not submit anything. | This makes the privacy and cost boundary visible before any model integration and prevents the MVP from implying that local evidence has already been approved for external submission. | No AI call, Stage Report, provider/model selection, secret scanning, redaction, Safety Gate enforcement, submit behavior, user approval flow, prompt construction, backend logic, or persistence. | Add Safety Gate, redaction checks, explicit user approval before submission, model/provider selection, structured Stage Report generation, and persistent review history. |
| Safety Gate / Redaction | Current MVP Safety Gate is a local frontend pattern scanner. It scans the serialized Stage Payload, including the currently selected file diff when that diff is loaded and included in the payload. It produces findings, status, and a redacted preview without mutating the original payload. No data is sent anywhere. | This makes privacy risk visible before AI integration while keeping implementation narrow, deterministic, and local-first. | The pattern scanner can miss secrets and can produce false positives. It does not scan all changed files, perform full-repo scanning, validate whether a detected string is live, provide backend enforcement, persist decisions, or support user-editable rules. | Safety Gate v2 should add local-only scanning of changed tracked files, optional bounded repository text-file scanning, ignore rules for `.git`, `node_modules`, `dist`, `target`, build outputs, large files, binaries, and lockfiles where appropriate, stronger secret patterns, optional entropy checks, configurable redaction rules, persisted decisions, and payload submission blocking until Safety Gate passes or the user explicitly overrides. This still does not mean sending the whole repo to an LLM. |
| Privacy and Safety | The app is local-first. There are no AI calls yet. Stage Payload and Staging Ground are local previews only. Secret redaction and Safety Gate enforcement are not implemented yet. | This keeps sensitive evidence local while the verification flow is still being built. | Payloads may include sensitive paths, diffs, stdout, stderr, or file names. No secret scanning, redaction, Safety Gate enforcement, or submit approval exists yet. | Add secret scanning, redaction preview, a Safety Gate before any LLM call, user approval before payload submission, and configurable privacy controls. |
...
| Retrieval, parsing, and RAG | No RAG, vector search, Tree-sitter, or codebase graph exists yet. | Retrieval should come after a working payload and token budget system. Staged should reduce context before adding advanced retrieval. | Payload context is based only on currently collected local evidence. There is no symbol-aware context expansion. RAG is not the same as full-repo scanning. | Add retrieval as a context-selection feature for AI review, starting with local search such as ripgrep. Later Tree-sitter or vector retrieval may add symbol and context awareness. RAG should select relevant snippets and reduce payload size, not expand it blindly or send whole repositories to an LLM by default. |
