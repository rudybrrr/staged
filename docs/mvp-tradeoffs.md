...
| Markdown Export | Milestone 11 implements local Markdown copy/export for the current local Stage Report. The export is generated from the current `StageReport`, uses browser clipboard APIs, and includes local-preview/no-AI wording, report metadata, repository/change summary, deterministic evidence, Safety Gate status, Token Budget estimate, command result summary when available, screening findings, payload limitations, risk findings, missing evidence, a human review checklist, and recommendation. | This makes the local report portable before AI integration or persistence exists, while keeping the export simple and inspectable. | Markdown copy/export is local report export only. It is not AI-generated, not persisted, not cloud-synced, and does not add backend behavior. No PDF or DOCX export, Stage History persistence, saved scan records, GitHub PR comment export, AI call, prompt construction, provider configuration, RAG implementation, new dependency, backend export logic, or Tauri capability change is included in the MVP. | Add Stage History persistence, saved scan records, Markdown/PDF export options, and GitHub PR comment export after the local MVP is stable. |
...
| Provider/API readiness and approval | No provider integration exists yet. There is no API key persistence, provider SDK/client code, provider/model selection, network request, AI call, prompt construction, or AI-generated report. Milestone 13 documents the future readiness and approval flow before real generation. | The app should make the external submission boundary explicit before adding provider code. MVP development should prefer environment-variable based provider readiness and should not store secrets in the repo, SQLite, or app-managed credential storage yet. | There is no real provider verification, credential storage, request execution, loading state, retry behavior, structured response validation, or Stage History persistence. A future `Generate AI Stage Report` action should remain disabled unless Safety Gate allows submission, a Stage Payload and redacted payload exist, and provider readiness is detected. | Add a provider readiness section in Staging Ground, explicit user approval, disabled-state reasons, redacted-payload-only submission, real structured AI Stage Report generation, provider/model configuration, secure credential storage, structured response validation, retry/error handling, and Stage History persistence. Verify official provider API documentation before implementing the real network call. |
