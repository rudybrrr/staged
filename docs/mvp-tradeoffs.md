...
| Token Budget Estimate | Planned MVP behavior is a local approximate estimate from payload size, including character count, byte count, estimated tokens, a heuristic such as `characters / 4`, section-level contribution breakdown, and warnings for large or missing sections. | This makes context size visible before any AI call and avoids provider/API coupling too early. | Not model-specific or exact. No provider pricing, tokenizer dependency, or automatic trimming. | Add model-aware tokenizer support, provider/model pricing, user-configurable token budgets, payload trimming controls, omitted-evidence warnings, and cost estimates before submission. |
