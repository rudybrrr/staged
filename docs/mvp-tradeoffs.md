...
| Safety Gate / Redaction | Milestone 9 MVP target is a frontend-only, local pattern-based scanner over the current Stage Payload only. It produces Safety Gate status, redaction findings, and a redacted payload preview without mutating the original Stage Payload. No data is sent anywhere. | This makes privacy risk visible before AI integration while keeping implementation narrow, deterministic, and local-first. | The pattern scanner can miss secrets and can produce false positives. It does not scan every file, validate whether a detected string is live, provide backend enforcement, persist decisions, or support user-editable rules. | Add backend Safety Gate enforcement, stronger secret scanning, entropy checks, configurable redaction rules, a redaction approval workflow, payload submission blocking until Safety Gate passes or is explicitly overridden, and persisted redaction decisions. |
