...
| Markdown Export | Milestone 11 defines local Markdown copy/export for the current local Stage Report. The export should include report metadata, repository summary, deterministic evidence, Safety Gate status, Token Budget summary, risk findings, missing evidence, a human review checklist, recommendation, and clear local-preview and not-AI-generated wording. | This makes the local report portable before AI integration or persistence exists, while keeping the export simple and inspectable. | Markdown copy/export is local report export only. No PDF or DOCX export, Stage History persistence, saved scan records, GitHub PR comment export, cloud sync, AI call, prompt construction, provider configuration, new dependency, backend export logic, or Tauri capability change is included in the MVP. Optional save-to-file should be avoided unless it can be done without expanding those boundaries. | Add Stage History persistence, saved scan records, Markdown/PDF export options, and GitHub PR comment export after the local MVP is stable. |
